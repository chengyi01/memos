import { o as observer, u as useTranslate, e as useCurrentUser, w as workspaceStore, R as Routes, j as jsxRuntimeExports, G as Link, y as Separator, B as Button, H as identityProviderServiceClient, J as extractIdentityProviderIdFromName, K as IdentityProvider_Type, h as absolutifyLink, n } from "./index.js";
...
const SignIn = observer(() => {
  ...
  const handleSignInWithIdentityProvider = async (identityProvider) => {
    ...
    if (identityProvider.type === IdentityProvider_Type.OAUTH2) {
      ...
      if (!oauth2Config) {
        n.error("Identity provider configuration is invalid.");
        return;
      }
      const authUrl = `${oauth2Config.authUrl}?client_id=${oauth2Config.clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${stateQueryParameter}&response_type=code&scope=${encodeURIComponent(
